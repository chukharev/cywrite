...
function process_token(token, cb) {
  ...
  var r = new CW.Clone({
    ...
    on_message_processed: function(channel, msg) {
      ...
      if (channel === 'act' && (msg.k === 'edit' || msg.k === 'split_paragraphs')) {
    ...
    on_broadcast: function() {
      var loctrans = function(row) {
        return row[8] === 'P' ? row[12] : row[8]
      }
